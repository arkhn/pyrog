# Migration `20200623144855-database-schema`

This migration has been generated by simonvadee at 6/23/2020, 2:48:55 PM.
You can check out the [state of the schema](./schema.prisma) after the migration.

## Database Steps

```sql
ALTER TABLE "pyrog"."AccessControl" ALTER COLUMN "role" DROP DEFAULT;

ALTER TABLE "pyrog"."Attribute" ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "definitionId" DROP DEFAULT,
ALTER COLUMN "path" DROP DEFAULT,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Column" DROP COLUMN "owner",
ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Comment" ALTER COLUMN "content" DROP DEFAULT,
ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Credential" ADD COLUMN "owner" text  NOT NULL DEFAULT E'',
ADD COLUMN "schema" text   ,
ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "database" DROP DEFAULT,
ALTER COLUMN "host" DROP DEFAULT,
ALTER COLUMN "login" DROP DEFAULT,
ALTER COLUMN "model" DROP DEFAULT,
ALTER COLUMN "password" DROP DEFAULT,
ALTER COLUMN "port" DROP DEFAULT,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Filter" ALTER COLUMN "relation" DROP DEFAULT,
ALTER COLUMN "value" DROP DEFAULT;

ALTER TABLE "pyrog"."Input" ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Join" ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Resource" DROP COLUMN "primaryKeyOwner",
ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "definitionId" DROP DEFAULT,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Source" DROP COLUMN "hasOwner",
ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "name" DROP DEFAULT,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."Template" ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "name" DROP DEFAULT,
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER TABLE "pyrog"."User" ALTER COLUMN "createdAt" SET DEFAULT CURRENT_TIMESTAMP,
ALTER COLUMN "email" DROP DEFAULT,
ALTER COLUMN "name" DROP DEFAULT,
ALTER COLUMN "password" DROP DEFAULT,
ALTER COLUMN "role" SET DEFAULT E'USER',
ALTER COLUMN "updatedAt" DROP DEFAULT;

ALTER INDEX "pyrog"."AccessControl.user_source" RENAME TO "AccessControl_user_source"

ALTER INDEX "pyrog"."Source.template_name" RENAME TO "Source_templateId_name"
```

## Changes

```diff
diff --git schema.prisma schema.prisma
migration ..20200623144855-database-schema
--- datamodel.dml
+++ datamodel.dml
@@ -1,0 +1,178 @@
+generator client {
+  provider = "prisma-client-js"
+}
+
+datasource db {
+  provider = "postgresql"
+  url      = env("POSTGRES_URL")
+}
+
+// One per software (Chimio, Crossway, etc)
+model Template {
+  id        String   @default(cuid()) @id
+  name      String   @unique
+  sources   Source[]
+  updatedAt DateTime @updatedAt
+  createdAt DateTime @default(now())
+}
+
+model Source {
+  id             String          @default(cuid()) @id
+  name           String
+  version        String?
+  resources      Resource[]
+  credential     Credential?
+  templateId     String          @map("template")
+  template       Template        @relation(fields: [templateId], references: [id])
+  updatedAt      DateTime        @updatedAt
+  createdAt      DateTime        @default(now())
+  accessControls AccessControl[]
+
+  @@unique([templateId, name], name: "Source_templateId_name")
+}
+
+enum DatabaseType {
+  POSTGRES
+  ORACLE
+}
+
+model Credential {
+  id        String       @default(cuid()) @id
+  host      String
+  port      String
+  database  String
+  login     String
+  password  String
+  owner     String       @default("")
+  schema    String?
+  model     DatabaseType
+  // Parent
+  sourceId  String       @map("source")
+  source    Source       @relation(fields: [sourceId], references: [id])
+  updatedAt DateTime     @updatedAt
+  createdAt DateTime     @default(now())
+}
+
+model Resource {
+  id               String      @default(cuid()) @id
+  label            String?
+  primaryKeyTable  String?
+  primaryKeyColumn String?
+  // filters on db to avoid processing all the DB
+  filters          Filter[]
+  attributes       Attribute[]
+  definitionId     String
+  sourceId         String      @map("source")
+  source           Source      @relation(fields: [sourceId], references: [id])
+  updatedAt        DateTime    @updatedAt
+  createdAt        DateTime    @default(now())
+}
+
+model Filter {
+  id          String    @default(cuid()) @id
+  relation    String
+  value       String
+  resourceId  String?   @map("resource")
+  resource    Resource? @relation(fields: [resourceId], references: [id])
+  sqlColumnId String    @map("sqlColumn")
+  sqlColumn   Column    @relation(fields: [sqlColumnId], references: [id])
+}
+
+model Attribute {
+  id            String    @default(cuid()) @id
+  path          String
+  sliceName     String?
+  definitionId  String
+  mergingScript String?
+  comments      Comment[]
+  inputs        Input[]
+  resourceId    String?   @map("resource")
+  resource      Resource? @relation(fields: [resourceId], references: [id])
+  updatedAt     DateTime  @updatedAt
+  createdAt     DateTime  @default(now())
+}
+
+model Comment {
+  id          String     @default(cuid()) @id
+  content     String
+  validation  Boolean    @default(false)
+  authorId    String     @map("author")
+  author      User       @relation(fields: [authorId], references: [id])
+  attributeId String?    @map("attribute")
+  attribute   Attribute? @relation(fields: [attributeId], references: [id])
+  updatedAt   DateTime   @updatedAt
+  createdAt   DateTime   @default(now())
+}
+
+model Input {
+  id           String    @default(cuid()) @id
+  // cleaning scripts for the sql column
+  script       String?
+  // id of the concept map to be applied to a mapped code
+  conceptMapId String?
+  // Or a static value is it's fixed
+  staticValue  String?
+  // parent
+  attributeId  String    @map("attribute")
+  attribute    Attribute @relation(fields: [attributeId], references: [id])
+  // Sql column containing the value
+  sqlValueId   String?   @map("sqlValue")
+  sqlValue     Column?   @relation(fields: [sqlValueId], references: [id])
+  updatedAt    DateTime  @updatedAt
+  createdAt    DateTime  @default(now())
+}
+
+model Column {
+  id        String   @default(cuid()) @id
+  table     String?
+  column    String?
+  joins     Join[]   @relation("ColumnJoins")
+  filter    Filter?
+  input     Input?
+  joinId    String?  @map("join")
+  join      Join?    @relation("JoinedColumns", fields: [joinId], references: [id])
+  updatedAt DateTime @updatedAt
+  createdAt DateTime @default(now())
+}
+
+model Join {
+  id        String   @default(cuid()) @id
+  tables    Column[] @relation("JoinedColumns")
+  columnId  String?  @map("column")
+  column    Column?  @relation("ColumnJoins", fields: [columnId], references: [id])
+  updatedAt DateTime @updatedAt
+  createdAt DateTime @default(now())
+}
+
+enum Role {
+  ADMIN
+  USER
+}
+
+model User {
+  id            String          @default(cuid()) @id
+  email         String          @unique
+  name          String
+  password      String
+  role          Role            @default(USER)
+  Comment       Comment[]
+  AccessControl AccessControl[]
+  updatedAt     DateTime        @updatedAt
+  createdAt     DateTime        @default(now())
+}
+
+enum SourceRole {
+  READER
+  WRITER
+}
+
+model AccessControl {
+  id       String     @default(cuid()) @id
+  role     SourceRole
+  userId   String     @map("user")
+  user     User       @relation(fields: [userId], references: [id])
+  sourceId String     @map("source")
+  source   Source     @relation(fields: [sourceId], references: [id])
+
+  @@unique([userId, sourceId], name: "AccessControl_user_source")
+}
```


