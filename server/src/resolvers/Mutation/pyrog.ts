...
export const pyrogMutation = {
  ...
  async createResourceTreeInSource(
    ...
    otherFhirResourceIntances = (
      await context.client.resources({
        where: {
    ...
    return fetch(
      `http://localhost:${process.env.SERVER_PORT}/resource/${resourceName}.json`,
    )
      .then((response: any) => {
        if (response.ok) {
  ...
  createAttributeProfileInAttribute(
    ...
    try {
      fetch(
        `http://localhost:${process.env.SERVER_PORT}/resource/${attributeType}.json`,
      )
        .then((response: any) => {
          return response.json()
        })
