...
export default class MainView extends React.Component<MimicViewReduxState, any> {
    public componentDidMount() {
        ...
        questions[question_index].sections[section_index].mapping_items.forEach((item: any) => {
            if (item.type) {
                this.props.dispatch(fetchBetaRecommendedColumns(
                    item.fhir_attribute,
                    item.type,
            ...
            <div id='mimic-poc'>
                ...
                {questions[question_index].sections[section_index].mapping_items.map((item: any, index: number) => {
                        ...
                        dataByAttribute[item.fhir_attribute].suggested_columns.map((column: any, index: number) => {
                            return <div key={index}>
                                <div
                                    ...
                                    onClick={() => this.props.dispatch(addInputColumn(item.fhir_attribute, column.column))}
                                >
                                    <Icon icon={'plus'}/>
                                </div>
                                <div className={'column'}>
                                    <div className={'column-header'}>
                                        {column.column.split('.').map((element: string, index: number) => {
                                            return <div key={index}>{(index+1) == column.column.split('.').length ? <strong>{element}</strong> : element.toUpperCase()}</div>
                                        })}
                                        <div>{Math.round(column.score * 100) / 100}</div>
                                    </div>
                                    <div className={'column-rows'}>
                                        {column.data.slice(1, 10).map((entry: string, index: number) =>
                                ...
                                return <div key={index}>
                                    <Breadcrumbs
                                        ...
                                        currentBreadcrumbRenderer={(item: any) => {
                                            return <div key={index}><strong>{item}</strong></div>
                                        }}
                                    />
                                </div>
                            }) :
                    ...
                    return <div className={'question'} key={index}>
                        <Callout className={'callout'} title={`${index + 1}. ${item.title}`}>
                            {item.text}
