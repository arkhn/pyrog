...
const AddResourceSelect = ({
  ...
  const renderItem: ItemRenderer<Resource> = (
    ...
    return (
      <TSelect<Profile>
        ...
        inputItem={resource}
        items={sortItems(resource.profiles || [])}
        onChange={onChange}
        popoverProps={{
          autoFocus: true,
          boundary: 'viewport',
  ...
  const sortAlphabetically = (item1: Profile, item2: Profile): number => {
  ...
  const sortItems = (profiles: Profile[]): Profile[] => {
    if (loading) {
      return [];
    }

    return profiles.sort((p1, p2) => sortAlphabetically(p1, p2));
  };
